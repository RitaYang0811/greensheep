<template>
  <div class="accordion-item my-5 bg-white" v-for="order in orders" :key="order.id">
    <h2 class="accordion-header" @click="toggleCollapse(order.id)">
      ...
      <button class="accordion-button fs-6 fw-bold bg-primary text-white" type="button" data-bs-toggle="collapse"
        :data-bs-target="`#${order.id}`" aria-expanded="true">
        訂單編號：{{ order.create_at }}
        <span :class="order.is_paid ? 'text-bg-primary' : 'text-bg-danger'"
          class="ms-10 badge rounded-pill text-bg-primary fs-8 fw-light">{{ order.is_paid ? '已付款' : '未付款' }}</span>
        <span class="ms-5 badge rounded-pill text-bg-primary fs-8 fw-light" v-if="order?.orderStatus?.done">已完成</span>
        <span class="ms-5 badge rounded-pill text-bg-danger fs-8 fw-light" v-if="order?.is_deleted">已刪除</span>
      </button>
    </h2>
        ...
        <div class="mb-4">
          <div class="fs-6 text-primary ms-3">
            ...
            <div class="d-flex justify-content-end">
              <button type="button" class="me-4 btn btn-deco" data-bs-target="#adOrderModal" @click="deleteOrder(order)"
                v-if="!order.is_deleted">
                刪除訂單</button><button type="button" class="me-4 btn btn-primary" data-bs-toggle="modal"
                data-bs-target="#adOrderModal" @click="openModal(order)">
                修改訂單
              </button>

              <button class="btn btn-danger me-4" v-if="order.is_deleted" @click.prevent="confirmDelete(order.id)">
                確認永久刪除訂單
              </button>
            </div>
          </div>
